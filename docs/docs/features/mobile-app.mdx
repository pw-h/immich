...
27-1-2025 This is incorrect in multiple ways:
- if i select a local album, i select a whole album, how can i selectively select photo's to upload if i select a whole album?.
- why would i have to select an album in the backup settings if i'm to selecively select photos in the main "Photo" screen?
- if i select a photo from a local folder, the following options present themselves: 
   - Share
  - Share with
  - Archive
  - Favorite
...
27-1-2025 Based on the above explanation from "Album Synchronization Highlights" onwards, i see almost no difference to selecting an extra album to Backup. The only difference seems that photos uploaded this way end up in an album on the server with the name of the local folder. That's not 'syncing', thats merely an specific option i would tick when i 'Backup' na specific folder, especially considering it's a one way upload, not a 'sync'. The whole title 'Sync' is misleading and confusing.
Furthermore, -> the cloud on the top leads to a screen "Back-up", it's not intuitive to press a cog wheel on the top right to get to 'sync options'. I then get to a screen "Backup-options". There was an option "Album sync" an hour ago, but it went completely missing. I only have two options "Automatic backup foreground" and "Automatic backup backgound" (translated form Dutch).  
